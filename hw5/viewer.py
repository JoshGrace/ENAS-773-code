...
class DoublePendViewer(Visualizer):
    ...
    def render(self, q):
        ...
        self._link2.set_transform(
            mc_trans.rotation_matrix(q[1], [1,0,0])
        )
        self._mass2.set_transform(mc_trans.translation_matrix([0,0,-l2]))


class BlockRigidBodyViewer(Visualizer):
    def __init__(self) -> None:
        Visualizer.__init__(self)
        body = self["ball"]
        _texture = body["texture"]
        _texture.set_transform(mc_trans.rotation_matrix(np.pi/2, [0,0,1]))
        _texture.set_object(
            mc_geom.Box([0.5,0.5,0.5]),
            mc_geom.MeshLambertMaterial(
                    # color=0x00ff00,
                    # opacity=0.5,
                    reflectivity=0.8,
    ...
    def render(self, q):
            px,pz,th = q
            _T = mc_trans.compose_matrix(
                            translate=[0,px,pz], 
                            angles=[th,0.,0.]
                )
            self.body.set_transform(_T)